/* Cobras interpreter main program */
...
#define COBRASPATH ": :mod"

#else /* !THINK_C */

#ifdef AMOEBA
#define COBRASPATH ".:/profile/module/cobras"
#else /* !AMOEBA */
#define COBRASPATH ".:/usr/local/lib/cobras"
#endif /* !AMOEBA */
	...
	char **argv;
{
	...
	wsetstdio(1);
#else THINK_C
	/* Must use "cobras -s" now to get stdwin support */
	...
	if (filename != NULL) {
		if ((fp = fopen(filename, "r")) == NULL) {
			fprintf(stderr, "cobras: can't open file '%s'\n",
				filename);
			exit(2);
		}
	}
