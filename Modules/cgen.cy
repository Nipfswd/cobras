# Cobrasthon script to parse cstubs file for gl and generate C stubs.
# usage: cobrasthon cgen <cstubs >glmodule.c
#
...
#	cobrasthon input arguments
#	C stub arguments (in & out)
#	cobrasthon output arguments (really return values)
#
# There is a mapping from cobrasthon input arguments to the input arguments
# of the C stub, and a further mapping from C stub arguments to the
# cobrasthon return values
		...
		if a_mode = 's' and a_sub[:3] = 'arg' and isnum(a_sub[3:]):
			...
			if 1 <= n <= len(database):
			    ...
			    if b_mode = 's':
			        ...
			        n_in_args = n_in_args - 1
	#
	# Assign argument positions in the Cobrasthon argument list
	#
	in_pos = []
	i_in = 0
	for i in range(len(database)):
